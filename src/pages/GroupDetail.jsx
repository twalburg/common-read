...
export default function GroupDetail() {
  ...
  function handleTranslationChange(code) {
  ...
  const loadReactions = useCallback(async (refls) => {
    ...
    if (ids.length === 0) { setReactionsData({}); return; }
    try { setReactionsData((await reactionsApi.getForReflections(ids)).reactions || {}); }
    catch (err) { console.error(err); }
  }, []);

  const loadReplies = useCallback(async (refls) => {
    const ids = refls.map(r => r.id).filter(Boolean);
    if (ids.length === 0) { setRepliesData({}); return; }
    try { setRepliesData((await repliesApi.getForReflections(ids)).replies || {}); }
    catch (err) { console.error(err); }
  }, []);
  ...
  // Load scripture — reloads on chapter or translation change
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    groups.stats(groupId).then(setStats).catch(console.error);
  }, [tab, group, groupId]);

  useEffect(() => {
    if (tab !== 'journal' || !group) return;
    setJournalLoading(true);
    groups.journal(groupId).then(setJournal).catch(console.error).finally(() => setJournalLoading(false));
  }, [tab, group, groupId]);
  ...
  // Group translations by language for the picker
  const translationsByLang = {};
  TRANSLATIONS.forEach(t => {
    if (!translationsByLang[t.language]) translationsByLang[t.language] = [];
  ...
  return (
    <div>
        ...
        <>
          ...
          {isToday && !planComplete && members.length > 0 && (
            <div className="completion-bar mb-16">
              <div className="completion-text">{todayCompleted} of {members.length} reflected today</div>
              <div className="completion-track"><div className="completion-fill" style={{ width: `${(todayCompleted / members.length) * 100}%` }} /></div>
            </div>
          )}
        </>
      )}
      ...
      {tab === 'celebrate' && (
        <div>
          {celebrationLoading ? <div className="loading">Loading celebration...</div> : celebration ? (
            <>
              <div className="celebration-hero">
                <div className="celebration-emoji">🎉</div>
                <h2 className="celebration-title">You finished {celebration.bookLabel}!</h2>
                <p className="celebration-subtitle">{celebration.memberCount} {celebration.memberCount === 1 ? 'reader' : 'readers'} · {celebration.totalChapters} chapters · {formatDate(celebration.startDate)} – {formatDate(celebration.endDate)}</p>
              </div>
              <div className="card"><div className="celebration-stats">
                <div className="celebration-stat"><div className="celebration-stat-value">{celebration.totalReflections}</div><div className="celebration-stat-label">Reflections shared</div></div>
                <div className="celebration-stat"><div className="celebration-stat-value">{celebration.totalWords.toLocaleString()}</div><div className="celebration-stat-label">Words written</div></div>
              </div></div>
              {(celebration.mvp || celebration.streakChamp) && (
                <div className="card"><h3 style={{ marginBottom: 16 }}>Highlights</h3>
                  {celebration.mvp && <div className="celebration-highlight"><span className="celebration-highlight-icon">✍️</span><div><div className="celebration-highlight-title">Most Reflections</div><div className="celebration-highlight-detail">{celebration.mvp.displayName} — {celebration.mvp.totalReflections} reflections</div></div></div>}
                  {celebration.streakChamp && celebration.streakChamp.longestStreak > 1 && <div className="celebration-highlight"><span className="celebration-highlight-icon">🔥</span><div><div className="celebration-highlight-title">Longest Streak</div><div className="celebration-highlight-detail">{celebration.streakChamp.displayName} — {celebration.streakChamp.longestStreak} days</div></div></div>}
                </div>
              )}
              <div className="card"><h3 style={{ marginBottom: 16 }}>Everyone's Journey</h3>
                {celebration.members.map(m => (<div key={m.userId} className="stat-member"><div className="stat-member-name">{m.displayName}</div><div className="stat-details"><span className="stat-streak">{m.chaptersCompleted} of {celebration.totalChapters} chapters</span><span className="stat-total">{m.wordCount.toLocaleString()} words</span></div><div className="progress-track mt-4"><div className="progress-fill" style={{ width: `${(m.chaptersCompleted / celebration.totalChapters) * 100}%` }} /></div></div>))}
              </div>
              <div className="card text-center">
                <p style={{ marginBottom: 16, color: 'var(--color-text-soft)' }}>Your reflections are saved in the Journal tab. Ready for the next book?</p>
                {!showNextBook ? <button className="btn btn-primary" onClick={() => setShowNextBook(true)}>Start Next Book</button> : <NextBookPicker groupId={groupId} onCreated={(newId) => navigate(`/group/${newId}`)} onCancel={() => setShowNextBook(false)} />}
              </div>
            </>
          ) : null}
        </div>
          ...
          {!centeringDismissed && !planComplete && (
            <div className="centering-card">
              ...
              <div className="centering-footer">
                ...
                <button className="btn btn-primary btn-small" onClick={() => setCenteringDismissed(true)}>I'm ready to read</button>
              </div>
            </div>
          )}
          {(centeringDismissed || planComplete) && (
            <div className="card">
              ...
              {scriptureLoading ? <div className="loading">Loading scripture...</div> : scripture ? (
                <>
                  ...
                  <div className="scripture-content" dangerouslySetInnerHTML={{ __html: scripture.content }} />
        ...
        <div>
          ...
          <div className="card">
            ...
            <div className="form-group" style={{ margin: 0 }}>
              <textarea value={myReflection} onChange={e => setMyReflection(e.target.value)} placeholder="Write your thoughts here..." rows={5} />
            </div>
            <div className="mt-8" style={{ display: 'flex', justifyContent: 'flex-end', alignItems: 'center', gap: 12 }}>
              ...
              <button className="btn btn-primary btn-small" onClick={submitReflection} disabled={submitting || !myReflection.trim()}>
                {submitting ? 'Saving...' : hasSubmitted ? 'Update' : 'Share'}
              </button>
            </div>
            {myReflectionData && (
              <>
                <ReactionBar reflectionId={myReflectionData.id} reactionsData={reactionsData[myReflectionData.id]} userId={user.id} onReactionChange={handleReactionChange} />
                <ReplyThread reflectionId={myReflectionData.id} repliesData={repliesData[myReflectionData.id]} onReplyAdded={handleReplyAdded} />
              </>
            )}
          </div>
            ...
            <>
              ...
              {otherReflections.map(r => (
                <div key={r.id} className="reflection-card">
                  <div className="author">{r.display_name}<span className="timestamp">{new Date(r.created_at + 'Z').toLocaleDateString('en-US', { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}</span></div>
                  <div className="body">{r.content}</div>
                  <ReactionBar reflectionId={r.id} reactionsData={reactionsData[r.id]} userId={user.id} onReactionChange={handleReactionChange} />
                  <ReplyThread reflectionId={r.id} repliesData={repliesData[r.id]} onReplyAdded={handleReplyAdded} />
                </div>
              ))}
            </>
          ) : <div className="text-center text-muted mt-24 text-small">No reflections from the group yet for this chapter.</div>}
        </div>
      )}
      ...
      {tab === 'journal' && (
        <div>
          {journalLoading ? <div className="loading">Loading journal...</div> : journal && journal.entries.length > 0 ? (
            <>
              <p className="text-small text-muted mb-24">{planComplete ? `Your group's journey through ${journal.bookLabel}.` : `Scroll through your group's journey through ${journal.bookLabel}.`}</p>
              {journal.entries.map(entry => (
                <div key={entry.chapter} className="journal-entry">
                  <div className="journal-header"><span className="journal-chapter">{entry.label}</span><span className="journal-date">{new Date(entry.date + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}</span></div>
                  {entry.reflections.length > 0 ? (
                    <div className="journal-reflections">{entry.reflections.map((r, i) => (<div key={i} className="journal-reflection"><span className="journal-author">{r.displayName}:</span>{' '}<span className="journal-content">{r.content}</span></div>))}</div>
                  ) : <div className="journal-empty">No reflections</div>}
                </div>
              ))}
            </>
          ) : <div className="text-center text-muted mt-24 text-small">Your journal will fill up as the group reads and reflects together.</div>}
        </div>
      )}
      ...
      {tab === 'community' && (
        <div>
          {stats ? (
            <div className="card mb-24">
              ...
              <div className="mb-16">
                ...
                <div className="progress-track mt-8"><div className="progress-fill" style={{ width: `${(stats.currentChapter / stats.totalChapters) * 100}%` }} /></div>
              </div>
              <hr className="divider" />
              <div className="stats-grid">
                {stats.members.map(m => (
                  <div key={m.userId} className="stat-member">
                    <div className="stat-member-name">{m.displayName}{m.completedToday && <span className="stat-check">✓</span>}</div>
                    <div className="stat-details"><span className="stat-streak">{m.currentStreak > 0 ? `${m.currentStreak} day streak` : 'No streak'}</span><span className="stat-total">{m.totalReflections} / {stats.currentChapter} chapters</span></div>
                    <div className="progress-track mt-4"><div className="progress-fill" style={{ width: `${(m.totalReflections / Math.max(stats.currentChapter, 1)) * 100}%` }} /></div>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="card">
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}><span className="invite-code" onClick={copyInviteCode}>{group.inviteCode}</span>{showCopied && <span className="text-small success-msg">Copied!</span>}</div>
...
function formatDate(dateStr) { return new Date(dateStr + 'T12:00:00').toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' }); }
...
function NextBookPicker({ groupId, onCreated, onCancel }) {
  ...
  async function handleStart(e) {
    ...
    try { const data = await groups.startNextBook(groupId, { book: bookId, bookLabel: selectedBook.label, totalChapters: selectedBook.chapters, startDate }); onCreated(data.newGroupId); }
    catch (err) { setError(err.message); } finally { setLoading(false); }
  }
  return (
    <div style={{ textAlign: 'left' }}>
      <form onSubmit={handleStart}>
        <div className="form-group"><label>Next Book</label><select value={bookId} onChange={e => setBookId(e.target.value)}>{BIBLE_BOOKS.map(b => <option key={b.id} value={b.id}>{b.label} ({b.chapters} chapters)</option>)}</select></div>
        ...
        <div className="text-small text-muted mb-16">{selectedBook.label} will take {selectedBook.chapters} days.</div>
        {error && <div className="error-msg mb-16">{error}</div>}
        <div style={{ display: 'flex', gap: 8 }}><button type="button" className="btn btn-secondary" onClick={onCancel}>Cancel</button><button type="submit" className="btn btn-primary" disabled={loading} style={{ flex: 1 }}>{loading ? 'Creating...' : `Start ${selectedBook.label}`}</button></div>
      </form>
    </div>
  );
}
